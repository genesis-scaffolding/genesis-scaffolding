...
PACKAGE_ROOT = Path(__file__).parent.resolve()  # Find the genesis_core directory in source
...
class PathConfigs(BaseModel):
    ...
    def internal_state_dir(self) -> Path:
        return self.working_directory / ".genesis"
...
class DatabaseConfig(BaseModel):
    dsn: str | None = None
    db_name: str = "genesis.db"
    echo_sql: bool = False
    db_directory: Path = Field(default_factory=lambda: Path.cwd() / ".genesis" / "database")
...
def get_config(
    ...
    if not conf.db.dsn:
        ...
        conf.db.db_directory = conf.path.server_root_directory / ".genesis" / "database"
        logger.debug("System DB DSN not provided, using path: %s", conf.db.db_directory)

    # User DB: Should always be inside the internal_state_dir (.genesis)
