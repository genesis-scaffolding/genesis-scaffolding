...
The frontend is designed as a **Backend-for-Frontend (BFF)**. Next.js does not serve as a standalone application but as an orchestration layer between the browser and the FastAPI backend (`genesis-server`).
