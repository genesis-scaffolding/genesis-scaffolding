...
The chat interface communicates with the backend via Server-Sent Events (SSE) at `/api/chats/{id}/stream`. For full streaming architecture details, see [genesis-server/sse-streaming.md](../genesis-server/sse-streaming.md).
...
- `genesis_frontend/app/` — App Router pages and layouts
- `genesis_frontend/app/actions/` — Server Actions
- `genesis_frontend/app/api/[...proxy]/` — API proxy route
- `genesis_frontend/components/dashboard/` — Dashboard components including page-container
- `genesis_frontend/components/dashboard/shared/data-table/` — TanStack Table engine
- `genesis_frontend/lib/api-client.ts` — API client utility
