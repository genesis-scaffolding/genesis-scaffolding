...
- `genesis_core.agent.clipboard` — Data models (`AgentClipboard`, `AgentClipboardFile`, `AgentClipboardToolResult`, `AgentClipboardTodoItem`, `AgentClipboardPinnedEntity`)
- `genesis_core.agent.agent_memory` — Clipboard integration (`AgentMemory` wraps `AgentClipboard`, provides `forget()`, TTL decay, entity pin/unpin, live-sync)
