...
- `genesis_core.agent` — Agent loop implementation (`step()`, turn management, loop flow)
- `genesis_core.agent.agent_memory` — Agent memory and clipboard integration (TTL decay via `forget()`, entity pinning)
- `genesis_core.agent.clipboard` — Clipboard data models (`AgentClipboard`, `AgentClipboardFile`, `AgentClipboardPinnedEntity`, `AgentClipboardToolResult`)
