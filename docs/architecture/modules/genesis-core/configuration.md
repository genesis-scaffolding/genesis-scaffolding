...
The base layer is populated from the process environment and the `.env` file in the working directory. Pydantic's `BaseSettings` handles both automatically. All environment variables carry the prefix `genesis__`. Nested configuration keys use double underscores (`__`) as separators. For example, `genesis__server__port` maps to `server.port`. This layer ensures the system can be bootstrapped from external infrastructure without any files on disk.
...
|---|---|---|---|
| System DB | `db` | Shared across all users | `server_root_directory/.genesis/database/genesis.db` |
...
| `internal_state_dir` | `working_directory / ".genesis"` |
...
```env
# Top-level
genesis__timezone=UTC
genesis__default_model=claude-3-5-sonnet
genesis__log_level=DEBUG  # DEBUG, INFO, WARNING, ERROR, CRITICAL

# Server
genesis__server__host=127.0.0.1
genesis__server__port=9000
genesis__server__cors_origins_extra=https://app.example.com,https://staging.example.com

# Database override (optional — SQLite is default)
genesis__db__dsn=postgresql://user:pass@localhost/genesis
```
